...
async def token(request: Request, provider: str):
    logger.debug(F"being called for provider {provider}")
    logger.debug(F"going to call  request.auth.clients[provider]._state: [{request.auth.clients[provider]._state}]")
    logger.debug(F" some info about: request.auth.clients[provider]: [{request.auth.clients[provider]}]")
    if request.auth.ssr:
